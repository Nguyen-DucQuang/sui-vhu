...
import { fetchUserNFTs } from '../../utils/sui-helpers';
...
const NFTUpload = () => {
  ...
  useEffect(() => {
    if (account?.address) {
      fetchUserNFTs(account.address).then(setUserNFTs);
    }
  }, [account]);
      ...
      const txb = {
        ...
        data: {
          ...
          arguments: [
            ...
            Array.from(new TextEncoder().encode("https://gateway.pinata.cloud/ipfs/Qm...")), // Mock URL
            listingConfig.price
          ],
          gasBudget: 10000000,
        },
      ...
      signAndExecuteTransaction(
        ...
        },
        {
          onSuccess: (result) => {
            ...
            // notify backend about the new transaction
            (async () => {
              try {
                ...
                if (txHash) {
                  await fetch(`${apiBase}/transactions/complete`, {
                    ...
                    body: JSON.stringify({
                      ...
                      amount: listingConfig?.price || 0,
                      currency: 'SUI',
                      status: 'completed',
                      metadata: { source: 'mint' }
                    })
                  });
                ...
                console.warn('Failed to report transaction to backend', e);
              }
            })();
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                    {userNFTs.length === 0 ? (
                      <p className="text-muted-foreground col-span-full text-center">
                        Bạn chưa có NFT nào. Tạo NFT đầu tiên của bạn ở trên!
                      </p>
                    ) : (
                      userNFTs.map((nft) => (
                        <div key={nft.objectId} className="bg-card border border-border rounded-xl p-4">
                          ...
                          <h4 className="text-sm font-medium mb-2">{nft.name}</h4>
